...
import { Calendar, Award, Users, Code, Leaf, Phone, Mail, ExternalLink, Zap, Globe, Brain } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-transparent overflow-hidden animate-fadeIn relative">
      ...
      <div className="text-center mt-5">
        <a href="https://forms.gle/cpJdoQpAtenTg8nL9" target="_blank" rel="nooreferrer">
          <button className="px-5 py-2 text-lg bg-blue-600 text-white border-none rounded cursor-pointer hover:bg-blue-700 transition-colors">
            Register Now
          </button>
        </a>
      </div>
            ...
            <div className="bg-white/80 border-4 border-green-300/40 rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
              <div className="p-10">
                <div className="flex justify-between items-start mb-8">
                  <div className="flex items-center space-x-4">
                    <div className={`p-4 bg-gradient-to-r ${selectedProblem.gradient} rounded-xl text-white shadow-lg`}>
                      {selectedProblem.icon}
                    </div>
                    <h3 className="text-4xl font-bold text-green-800">{selectedProblem.title}</h3>
                  </div>
                  <button
                    onClick={() => setSelectedProblem(null)}
                ...
                <div className="prose prose-lg max-w-none mb-8">
                  <p className="text-green-700/90 leading-relaxed text-xl">
                    {selectedProblem.details}
                  </p>
                </div>
                ...
                <div className="flex flex-wrap gap-4 mb-8">
                  {selectedProblem.tags.map((tag, index) => (
                    <span
                      key={index}
                      className={`px-6 py-3 bg-gradient-to-r ${selectedProblem.gradient} text-white rounded-full font-bold shadow-lg`}
                    >
                      {tag}
                    </span>
                  ))}
